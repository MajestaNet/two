...
DevFlow is a small control plane: CLI, API, scheduler, worker, SQLite, optional
Slack adapter.

## Decision

Implement DevFlow in Python 3.12+ managed by `uv`. Runtime dependency for the
...
- Packaging is a src-layout installable `devflow` package with a `devflow`
  console script.
