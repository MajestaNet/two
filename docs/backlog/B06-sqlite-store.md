...
1. **Schema** (migrations in `src/two/store/`, versioned)
...
2. **Engine**
   ...
   - Path under `TWO_DATA_DIR`. Create directories.
...
You are implementing **Majesta Two backlog item B06 — SQLite store, events, and leases**.
...
4. `src/two/store/__init__.py`, `src/two/types.py`
